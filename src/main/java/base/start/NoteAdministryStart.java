...
public class NoteAdministryStart {
    ...
    private static void mapCommandExecution(Command activeCommand) {
        switch (activeCommand.getComandName()) {
            ...
            case "search overview" -> new OverviewWordFinder();
            case "read" -> new NoteReader();
            case "send" -> new SingleNoteDispatcher();
            case "single note sc" -> new SingleNoteSpellChecker();
            case "single word sc" -> new SingleWordSpellchecker();
            case "overview sc" -> new OverviewSpellChecker();
    ...
    private static void fillCommands() {
        ...
        commandList.add(new Command("search overview", "get information about the occurence of a keyword within all notes"));
        commandList.add(new Command("send", "send a note via email"));
        commandList.add(new Command("single note sc", "check single word/s for spelling"));
        commandList.add(new Command("single word sc", "check the spelling for a specific note"));
        commandList.add(new Command("overview sc", "check the spelling for all files in directory" + path));
        commandList.add(new Command("so", "nice"));
